...
namespace CVNetControls
{
	...
	public ref class TitledCheckBoxTitledList : public ControlTitledList<TitledCheckBox>
	{
		public:
		...
		#pragma endregion
			...
			TitledCheckBoxTitledList() : ControlTitledList<TitledCheckBox>() { _locked = false; _multiSelect = true; }
			TitledCheckBoxTitledList(Control^ parent) : ControlTitledList<TitledCheckBox>(parent) { _locked = false; _multiSelect = true; }
			...
			TitledCheckBox^ addCheckBox(String^ name, String^ title, bool checked)
			{
				...
				box->Checked = checked;
				box->OffsetX = 5;
				box->addMouseDownEvent(gcnew MouseEventHandler(this, &TitledCheckBoxTitledList::Event_MouseDown));
				return add(box);
			}
