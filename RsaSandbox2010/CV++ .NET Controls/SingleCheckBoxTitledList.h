...
namespace CVNetControls
{
	...
	public ref class SingleCheckBoxTitledList : public ControlTitledList<SingleCheckBox>
	{
		public:
		...
		#pragma endregion
			...
			SingleCheckBoxTitledList() : ControlTitledList<SingleCheckBox>() { _locked = false; _multiSelect = true; }
			SingleCheckBoxTitledList(Control^ parent) : ControlTitledList<SingleCheckBox>(parent) { _locked = false; _multiSelect = true; }
			...
			SingleCheckBox^ addCheckBox(String^ name, bool checked)
			{
				...
				box->Name = name;
				box->Checked = checked;
				box->addMouseDownEvent(gcnew MouseEventHandler(this, &SingleCheckBoxTitledList::Event_MouseDown));
				return add(box);
			}
