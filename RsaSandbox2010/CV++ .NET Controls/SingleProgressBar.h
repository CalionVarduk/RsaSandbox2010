...
namespace CVNetControls
{
	...
	public ref class SingleProgressBar : public SingleControl<DoubleBufferedPanel>
	{
		public:
...
#pragma endregion
			...
			SingleProgressBar() : SingleControl<DoubleBufferedPanel>()
			{ _initialize(); }
			
			SingleProgressBar(Control^ parent) : SingleControl<DoubleBufferedPanel>()
			{
				Parent = parent;
				_initialize();
			}

			SingleProgressBar(Control^ parent, String^ name) : SingleControl<DoubleBufferedPanel>()
			{
				Name = name;
				Parent = parent;
				_initialize();
		...
		private:
			...
			void _initialize()
			{
				...
				_control->Paint += gcnew PaintEventHandler(this, &SingleProgressBar::Event_OnPaint);
			}
