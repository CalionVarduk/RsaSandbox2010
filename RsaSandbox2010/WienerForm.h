...
namespace RsaSandbox
{
	...
	public ref class WienerForm : public SecondaryForm
	{
		public:
			WienerForm(LabelButton^ controlButton, KeyGenForm^ keyGenForm) : SecondaryForm(controlButton)
			{
				this->Text = L"RSA Sandbox : Wiener's Attack";
				...
				wiener = gcnew WienerAttack();
				wiener->ReportProgress += gcnew TaskReportProgressHandler(this, &WienerForm::Event_AttackReportProgress);
				wiener->Outcome += gcnew WienerAttackOutcomeHandler(this, &WienerForm::Event_AttackOutcome);

				faultyGen = gcnew FaultyKeyGenerator();
				faultyGen->Outcome += gcnew FaultyGenOutcomeHandler(this, &WienerForm::Event_FaultyGenOutcome);
			}
...
#pragma endregion
			...
			void generateFaultyKey(Int32 modBitLength, Int32 privateExponentBitLength)
			{
				hideUI(true);
				faultyGen->wienerUnsafe(modBitLength >> 1, privateExponentBitLength);
			}

			void prepareAndAttack()
			{
				prepareAndLockUI();
				wiener->tryToBreak(key);
			}
			...
			void Event_AttackOutcome(WienerAttack^ sender, RSAKey^ brokenKey, WienerAttackOutcome outcome)
			{
				if(outcome == WienerAttackOutcome::OK) {
					key = brokenKey;
					prepareAndUnlockUI(true);
				}
				else {
					prepareAndUnlockUI(false);
					if(outcome == WienerAttackOutcome::Unsuccessful)
						SimpleBox::display("Provided RSA Key appears to be unbreakable via Wiener's Attack.", "Wiener's Attack");
				}
			}
...
#pragma region UI Events
			...
			void prepareAndLockUI()
			{
				...
				Toolbox::setTitledControl(informant, "Performing Wiener's attack:", "0%");
				...
				ClientSize = System::Drawing::Size(keyNumbers->Right + 12, progressBar->Bottom + 12);
				ResumeDraw;
			}
			...
			void prepareAndUnlockUI(bool ok)
			{
				...
				if(ok) {
					Toolbox::setTitledControl(informant, "Time taken:", wiener->TimeElapsedMs.ToString("N0") + " ms");
...
#pragma region User Events
			...
			{
				if(!wiener->IsBusy && !faultyGen->IsBusy) {
			...
			{
				if(!wiener->IsBusy && !faultyGen->IsBusy) {
					WienerFaultyKeyForm^ form = gcnew WienerFaultyKeyForm();
					System::Windows::Forms::DialogResult result = form->ShowDialog();

					if(result == System::Windows::Forms::DialogResult::OK) {
						generateFaultyKey(form->ModulusBitLength, form->PrivateExponentBitLength);
					}
				}
			...
			void Event_PerformAttack(Object^ sender, MouseEventArgs^ e)
			{
				if(!wiener->IsBusy) {
					...
					if(key->IsWienerSafe) {
						...
						msg[0] = "The RSA Key is flagged as safe against Wiener's Attack.";
...
#pragma region Initialization
			...
			void _initializeInformant()
			{
				...
				progressBar->BorderColor = clearButton->BackColorNormal;
				progressBar->BackColor = Color::Azure;
				progressBar->ForeColor = clearButton->BackColorNormal;
				progressBar->Top = informant->Bottom + 5;
				progressBar->Left = informant->Left;
				progressBar->Visible = false;
			}
			...
			{
				menuButtons[0]->addMouseDownEvent(gcnew MouseEventHandler(this, &WienerForm::Event_LoadKey));
				menuButtons[1]->addMouseDownEvent(gcnew MouseEventHandler(this, &WienerForm::Event_ImportKey));
				menuButtons[2]->addMouseDownEvent(gcnew MouseEventHandler(this, &WienerForm::Event_GenFaultyKey));

				clearButton->addMouseDownEvent(gcnew MouseEventHandler(this, &WienerForm::Event_ClearKey));
				performAttack->addMouseDownEvent(gcnew MouseEventHandler(this, &WienerForm::Event_PerformAttack));
			}
	};
}